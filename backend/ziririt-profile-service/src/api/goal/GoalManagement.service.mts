...
export class GoalManagementService {
  ...
  async updateGoal(goalId: string, userId: string, updates: UpdateGoalData): Promise<Goal> {
  ...
  async getGoalById(goalId: string, viewerId?: string): Promise<Goal | null> {
    ...
    const result = await this.pool.query<Goal & { user_display_name: string }>(query, params);
